...
class HermesBrain:
    ...
    def think(self, user_text: str) -> str:
        ...
        for model_name in self.models_to_try:
            for attempt in range(2): # Try each model up to 2 times with backoff
                try:
                    ...
                    if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                        print(f"[Brain Quota Notice]: Rate limit hit on {model_name}. Pausing for 5s...")
                        time.sleep(5.0) # Backoff cooldown
                    else:
                        print(f"[Brain Notice]: Model {model_name} failed: {e}")
                        break
            if reply:
                break
            ...
            for model_name in self.models_to_try:
                try:
                    ...
                    if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                        print(f"[Vision Quota Notice]: Rate limit hit on {model_name}. Pausing for 5s...")
                        time.sleep(5.0)
                    continue
            return "Sir, visual analysis is temporarily rate-limited. Please try again shortly."
        except Exception as e:
        ...
        try:
            ...
            for model_name in self.models_to_try:
                try:
                    ...
                    print(f"[Vision Debug - Model '{model_name}' failed]: {e}")
                    time.sleep(0.2)
                    continue
            return "Sir, visual analysis failed across all active endpoints."
        except Exception as e:
            return f"[Eyes System Error]: {e}"